...
/*the "letter token" gets matched first (in scanner) for a single letter word. so need a new type id to account for that. */
id:
		ID { $1 }
	| LETTER { $1  }
...
/*will need to update this to include the datatype in the action 
TODO: add 'datatype' after "COMMA id" */
formal_list:
